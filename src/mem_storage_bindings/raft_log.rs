...
use crate::bindings::unstable::PyUnstableRef;
use crate::raftpb_bindings::entry::{PyEntry, PyEntryMut, PyEntryRef};

#[pyclass(name = "InMemoryRaftLog")]
pub struct PyInMemoryRaftLog {
    pub inner: RefMutOwner<RaftLog<MemStorage>>,
}

#[pyclass(name = "InMemoryRaftLogRef")]
pub struct PyInMemoryRaftLogRef {
    pub inner: RefMutContainer<RaftLog<MemStorage>>,
}

#[pymethods]
impl PyInMemoryRaftLog {
    #[new]
    pub fn new(store: PyMemStorageMut, logger: PyLoggerMut) -> Self {
        PyInMemoryRaftLog {
            inner: RefMutOwner::new(RaftLog::new(store.into(), logger.into())),
        }
    }

    pub fn make_ref(&mut self) -> PyInMemoryRaftLogRef {
        PyInMemoryRaftLogRef {
...
impl PyInMemoryRaftLogRef {
    ...
    pub fn get_store(&mut self) -> PyResult<PyMemStorageRef> {
        self.inner.map_as_mut(|inner| PyMemStorageRef {
            inner: RefMutContainer::new_raw(inner.mut_store()),
        })
    }
